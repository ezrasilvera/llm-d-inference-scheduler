...
**nokube mode** lets the EPP run on any machine — bare metal, a VM, a container on a
developer laptop — without a Kubernetes API server. Backend discovery is handled by a
pluggable `BackendDiscovery` implementation instead of watching K8s pods.

The table below summarises the key behavioural differences between modes:
...
Everything else — scheduling algorithms, flow control, metrics polling from inference
engines, Prometheus metrics, TLS, health checking — works identically in both modes.
...
> binary when that work is undertaken. Level 2 is documented here as a future hardening
> option only — it is **not** the current or planned implementation.
...
- Lifecycle is managed by `errgroup` rather than `ctrl.Manager`.
- K8s packages (`k8s.io/api`, `sigs.k8s.io/controller-runtime`) are still compiled into
  the binary but are only used for logging helpers (which have no API server dependency).

**Binary model:** a **single binary** with two execution paths. Mode is selected
automatically: if `backendDiscovery` is present in the config, nokube path runs; otherwise
the standard K8s path runs. `cmd/epp-nokube/` is a thin 15-line entrypoint that calls the
same runner with nokube mode pre-selected.
...
management, gRPC server startup, and Prometheus metrics serving — all of which already
exist in the K8s runner, producing ~185 lines of duplication.

**Duplication introduced by Level 2 vs Level 1:**
...
|---|---|---|
| Runs without K8s cluster at runtime | ✓ | ✓ |
| No K8s package imports in binary | ✗ | ✓ |
| Binary size | ~10 MB larger than Level 2 | Smaller |
| Supply-chain / SBOM cleanliness | K8s packages present | Clean |
| Code duplication | ~0 lines | ~185 lines |
| Separate binary entrypoint required | Thin shim only | Full independent runner |
| Separate `Dockerfile.epp-nokube` | ✓ | ✓ |

Both levels produce a separate `epp-nokube` container image that works without a cluster.
The trade-off is binary cleanliness (Level 2) vs code simplicity and zero duplication
(Level 1).
...
**Two images** — separate `epp` (K8s) and `epp-nokube` images built from the same
codebase via separate Dockerfiles. Each image does exactly one thing; the name is
self-documenting.
...
2. **Deployment scenario alignment** — nokube targets bare-metal, edge, and VM deployments
   where K8s client packages in the binary may be undesirable even at Level 1.
...
| `Labels` | Arbitrary key-value labels used by scheduling filters (e.g. role, model name) |

### Discovery lifecycle contract

1. `Start` is called once; it blocks until `ctx` is cancelled.
2. On startup, the implementation calls `Upsert` for every currently-known backend, then calls `MarkSynced`. Until `MarkSynced` is called, the EPP health check is not ready.
3. After the initial sync, the implementation continues watching and calls `Upsert` / `Delete` as backends appear or disappear.
4. Exactly one `BackendDiscovery` plugin may be configured; the loader enforces this.

### Selecting a plugin via config

`BackendDiscovery` follows the same two-level plugin pattern used by `Parser`,
`SaturationDetector`, and `DataLayer`:

1. **Declare a plugin instance** in the top-level `plugins` list, giving it a type, name, and parameters.
...
of backend objects. On each poll, the full list is diffed against the current set —
new entries are upserted and missing entries are deleted.
...
with exponential backoff (2 s → 30 s cap) on connection failure.
...
**Wire format** — `BackendEvent` JSON (sent as gRPC messages):
...
Supports two modes: SRV records (richer, preferred) and A/AAAA records (simpler).

**Parameters:**

| Parameter | Type | Default | Description |
|---|---|---|---|
| `dnsMode` | `"srv"` \| `"a"` | `"srv"` | Resolution strategy |
...
Both fields are optional. Omitting them is safe:
- No `InferenceObjective` → all requests use priority `0`.
- No `InferenceModelRewrite` → model names pass through unchanged.
...
| `k8s-notification-source` plugins | No K8s API server; config validation will fail fast |
